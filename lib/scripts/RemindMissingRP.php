...
class RemindMissingRP extends AbstractScript {
  ...
  public function run() {
    if (DB::g(STN::MAIL_MISSING_RP_REMINDER) === null) {
      self::errln("No e-mail template for missing RP reminder (MAIL_UNFINALIZED_REMINDER).");
      return;
    }
    ...
    foreach ($missing as $user_id => $list) {
      ...
      self::errln(sprintf("Sent email to %s (%s) regarding %d regatta(s).", $user, $user->id, count($list)));
    }
  }

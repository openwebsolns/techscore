 ...
 *
 * Provides functionality for ordering teams based on score
 *
 * @author Dayan Paez
 * @version 2013-01-10
 */
class WinLossRecord {
   ...
   */
  public function __construct(Team $team, $wins, $losses, $ties = 0) {
    $this->team = $team;
    $this->wins = (int)$wins;
  ...
  public function __set($name, $value) {
    if ($name != 'team' || !($value instanceof Team))
      throw new InvalidArgumentException("Property $name does not exist, or invalid type.");
    $this->$name = $value;
  }
   ...
   *
   * @param WinLossRecord $a the first team
   * @param WinLossRecord $b the second team
   * @return int < 0 if first team ranks higher, > 0 otherwise
   */
  public static function compare(WinLossRecord $a, WinLossRecord $b) {
